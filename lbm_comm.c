...
void lbm_comm_sync_ghosts_horizontal( lbm_comm_t * mesh, Mesh *mesh_to_process, lbm_comm_type_t comm_type, int target_rank, int x )
{
	...
	// {
	// 	case COMM_SEND:
	// 			MPI_Send( Mesh_get_col( mesh_to_process, x ), DIRECTIONS*mesh->height, MPI_DOUBLE, target_rank, 0, MPI_COMM_WORLD);
	// 			// MPI_Isend( Mesh_get_col( mesh_to_process, x ), DIRECTIONS*mesh->height, MPI_DOUBLE, target_rank, 0, MPI_COMM_WORLD, &(mesh->requests[target_rank]));

	// 		break;
	// 	case COMM_RECV:
	// 			MPI_Recv( Mesh_get_col( mesh_to_process, x ), DIRECTIONS*mesh->height, MPI_DOUBLE, target_rank, 0, MPI_COMM_WORLD,&status);
	// 		break;
	// 	default:
	...
	switch (comm_type)
	{
		case COMM_SEND:
			...
			break;
		case COMM_RECV:
				MPI_Irecv( Mesh_get_col( mesh_to_process, x ), DIRECTIONS*mesh->height, MPI_DOUBLE, target_rank, 0, MPI_COMM_WORLD, &(mesh->requests[target_rank]));
				// MPI_Recv( Mesh_get_col( mesh_to_process, x ), DIRECTIONS*mesh->height, MPI_DOUBLE, target_rank, 0, MPI_COMM_WORLD,&status);
			break;
		default:
			fatal("Unknown type of communication.");

	// MPI_Wait(&(mesh->requests[target_rank]), MPI_STATUS_IGNORE);
	}
}

/*******************  FUNCTION  *********************/
/**
 * Attente de communications asynchrones
 * @param mesh_comm MeshComm à utiliser
 * @param target_rank processus recevant le message
**/
void lbm_comm_sync_ghosts_horizontal_wait(lbm_comm_t * mesh)
{
	//if target is -1, no comm
	if (mesh->left_id == -1 || mesh->right_id == -1)
		return;

	int flag1, flag2;
	while(MPI_Test(&(mesh->requests[mesh->left_id]), &flag1, MPI_STATUS_IGNORE) &&
		MPI_Test(&(mesh->requests[mesh->right_id]), &flag2, MPI_STATUS_IGNORE));
}
...
void lbm_comm_sync_ghosts_diagonal( lbm_comm_t * mesh, Mesh *mesh_to_process, lbm_comm_type_t comm_type, int target_rank, int x , int y)
{
	...
	switch (comm_type)
	{
		case COMM_SEND:
			...
			// MPI_Isend( Mesh_get_cell( mesh_to_process, x, y ), DIRECTIONS, MPI_DOUBLE, target_rank, 0, MPI_COMM_WORLD, &(mesh->requests[0]));
			break;
		case COMM_RECV:
		...
		case COMM_SEND:
			for ( x = 1 ; x < mesh_to_process->width - 2 ; x++)
				for ( k = 0 ; k < DIRECTIONS ; k++)
					...
					// MPI_Isend( &Mesh_get_cell(mesh_to_process, x, y)[k], 1, MPI_DOUBLE, target_rank, 0, MPI_COMM_WORLD, &(mesh->requests[0]));
			break;
		case COMM_RECV:
			for ( x = 1 ; x < mesh_to_process->width - 2 ; x++)
				for ( k = 0 ; k < DIRECTIONS ; k++)
